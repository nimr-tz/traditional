...
export default function EmailCampaignShow({ campaign, recipients }: { campaign: Campaign; recipients: Paginated<Recipient> }) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Admin', href: '/admin' },
        { title: 'Email Manager', href: '/admin/emails' },
        { title: campaign.subject, href: route('admin.emails.show', campaign.id) },
    ];
            ...
            <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 pb-10 md:p-6">
                <header className="flex items-start gap-4 border-b pb-5">
                    ...
                    <div className="min-w-0">
                        ...
                        <h1 className="mt-2 font-serif text-3xl font-semibold">{campaign.subject}</h1>
                ...
                <section className="bg-card rounded-2xl border p-5">
                    ...
                    <div className="mt-3 text-[15px] leading-8 whitespace-pre-wrap text-slate-800 dark:text-slate-200">{campaign.body}</div>
                </section>
                            ...
                            <tbody className="divide-y">
                                {recipients.data.map((recipient) => (
                                    <tr key={recipient.id} className="hover:bg-muted/30">
                                        ...
                                        <td className="text-muted-foreground p-4">{recipient.email}</td>
                                        <td className="p-4">
